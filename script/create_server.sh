...
case $no in
        ...
        "2" )
                ...
                mariadb -u root -p$pw $db_name -e "create table TBL_MEMORY_CHECK(ILJA datetime not null comment '일자', TOTAL_MEMORY int not null comment '총_메모리', USED_MEMORY int not null comment '사용중인_메모리', FREE_MEMORY int not null comment '사용가능_메모리', SEVER_MEMORY_USAGE int not null comment '메모리_사용률', DBMS_TOTAL_MEMORY int not null comment 'DBMS_허용된_메모리크기',DBMS_MEMORY_CACHE_CHECK int not null comment 'DBMS_메모리_캐시사용률', CONSTRAINT PRIMARY KEY (ILJA desc)) comment 'MEMORY_모니터링';"
                mariadb -u root -p$pw $db_name -e "create table TBL_STORAGE_CHECK(ILJA datetime not null comment '일자', DRIVE_NAME char(1) not null comment '드라이브_명', TOTAL_STORAGE_GB int null comment '총_용량', USED_STORAGE_GB int null comment '사용중인_용량', FREE_STORAGE int null comment '사용가능_용량', CONSTRAINT PRIMARY KEY (ILJA desc,DRIVE_NAME)) comment 'STORAGE_모니터링';"
                echo "테이블 생성완료"
